...
#define PLUGIN_ERROR(message)                                            \
    std::cerr << "[" << std::this_thread::get_id() << "] [ERROR] [SDF] " \
              << message << std::endl;
#define PLUGIN_WARN(message)                                             \
    std::cerr << "[" << std::this_thread::get_id() << "] [WARN ] [SDF] " \
              << message << std::endl;
#define PLUGIN_INFO(message)                                             \
    std::cout << "[" << std::this_thread::get_id() << "] [INFO ] [SDF] " \
              << message << std::endl;
#ifdef NDEBUG
#define PLUGIN_DEBUG(message)
#else
#define PLUGIN_DEBUG(message)                                            \
    std::cout << "[" << std::this_thread::get_id() << "] [DEBUG] [SDF] " \
              << message << std::endl;
#endif
